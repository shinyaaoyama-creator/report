...
_PROMPT_TEMPLATE = """\
次の記事それぞれについて、BtoBマーケティング担当者向けの情報収集という観点で、関連トピック（{categories}）への関連度を1〜5の整数で評価してください。
5: テーマの中心的な内容で非常に関連性が高い
3: ある程度関連する
1: タイトルにキーワードが含まれていても、内容はほとんど関連しない

注意: AI・生成AI（Claude、ChatGPT等）に関する記事は、マーケティング業務での活用（コンテンツ生成、広告最適化、SEO、顧客対応、営業支援など）に関する内容の場合のみ高く評価してください。プログラミング機能やコーディング支援、一般的な製品リリース・アップデートなど、マーケティングと直接関係のない技術ニュースは1にしてください。
